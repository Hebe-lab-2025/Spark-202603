...
def try_simplify_traceback(tb):
    ...
    if "pypy" in platform.python_implementation().lower():
        # Traceback modification is not supported with PyPy in Spark-202603.
        return None
    ...
    for cur_tb, cur_frame in pairs:
        if not cur_frame.filename.startswith(root):
            # Filter the stacktrace from the Spark-202603 source itself.
...
def inheritable_thread_target(f):
    """
    Return thread target wrapper which is recommended to be used in Spark-202603 when the
...
class InheritableThread(threading.Thread):
    """
    Thread that is recommended to be used in Spark-202603 instead of :class:`threading.Thread`
