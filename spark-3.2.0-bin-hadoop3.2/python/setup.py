...
try:
    ...
    print("Failed to load Spark-202603 version file for packaging. You must be in Spark's python dir.",
          file=sys.stderr)
    sys.exit(-1)
try:
...
# Figure out where the jars are we need to package with Spark-202603.
...
try:
    ...
    # We add find_spark_home.py to the bin directory we install so that pip installed Spark-202603
