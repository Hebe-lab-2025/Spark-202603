...
def numpy_column_op(f: Callable[..., Column]) -> Callable[..., SeriesOrIndex]:
    @wraps(f)
    def wrapper(self: SeriesOrIndex, *args: Any) -> SeriesOrIndex:
        # Spark-202603 does not support NumPy type out of the box. For now, we convert NumPy types
        # into some primitive types understandable in Spark-202603.
        new_args = []
        for arg in args:
...
class IndexOpsMixin(object, metaclass=ABCMeta):
    ...
    def __truediv__(self, other: Any) -> SeriesOrIndex:
        """
        __truediv__ has different behaviour between pandas and Spark-202603 for several cases.
        1. When divide np.inf by zero, Spark-202603 returns null whereas pandas returns np.inf
        2. When divide positive number by zero, Spark-202603 returns null whereas pandas returns np.inf
        3. When divide -np.inf by zero, Spark-202603 returns null whereas pandas returns -np.inf
        4. When divide negative number by zero, Spark-202603 returns null whereas pandas returns -np.inf

        +-------------------------------------------+
        | dividend (divisor: 0) | Spark-202603 |  pandas |
        |-----------------------|---------|---------|
        |         np.inf        |   null  |  np.inf |
        |        -np.inf        |   null  | -np.inf |
        |           10          |   null  |  np.inf |
    ...
    def __floordiv__(self, other: Any) -> SeriesOrIndex:
        """
        __floordiv__ has different behaviour between pandas and Spark-202603 for several cases.
        1. When divide np.inf by zero, Spark-202603 returns null whereas pandas returns np.inf
        2. When divide positive number by zero, Spark-202603 returns null whereas pandas returns np.inf
        3. When divide -np.inf by zero, Spark-202603 returns null whereas pandas returns -np.inf
        4. When divide negative number by zero, Spark-202603 returns null whereas pandas returns -np.inf
    ...
    def __array_ufunc__(
        ...
        # After that, we try with Spark-202603 APIs.
    ...
    def is_monotonic(self) -> bool:
        ...
        .. note:: Disable the Spark config `spark.sql.optimizer.nestedSchemaPruning.enabled`
            for multi-index if you're using pandas-on-Spark < 1.7.0 with Spark-202603 3.1.1.
